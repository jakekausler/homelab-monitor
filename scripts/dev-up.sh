...
# ----------------------------------------------------------------------------
ensure_data_dirs() {
  # Base dir for dev-rig scratch state (seed compose, build-contexts, backups,
  # config). Parameterized so two dev rigs on the same host don't collide on
  # /tmp/hm-dev. Default-assigned HERE (not at top-level) because load_env()
  # sources deploy/dev/dev.env from inside main() BEFORE this function runs —
  # so a value set in dev.env wins over the /tmp/hm-dev default.
  HM_DEV_BASE="${HM_DEV_BASE:-/tmp/hm-dev}"

  mkdir -p "${LOG_DIR}"
  mkdir -p "${HM_DEV_BASE}/backup" "${HM_DEV_BASE}/runbook-transcripts"
  ...
  mkdir -p "${HM_DEV_BASE}/compose/udo-viewer"
  if [[ ! -f "${HM_DEV_BASE}/compose/docker-compose.yml" ]]; then
    cat > "${HM_DEV_BASE}/compose/docker-compose.yml" <<'EOF'
...
EOF
  ...
  if [[ ! -f "${HM_DEV_BASE}/compose/udo-viewer/Dockerfile" ]]; then
    cat > "${HM_DEV_BASE}/compose/udo-viewer/Dockerfile" <<'EOF'
FROM alpine:3.20
...
EOF
  ...
  mkdir -p "${HM_DEV_BASE}/config/docker"
  if [[ ! -f "${HM_DEV_BASE}/config/docker/build-sources.yaml" ]]; then
    cat > "${HM_DEV_BASE}/config/docker/build-sources.yaml" <<EOF
...
compose_files:
  - host_path: ${HM_DEV_BASE}/compose/docker-compose.yml
    container_path: ${HM_DEV_BASE}/compose/docker-compose.yml
build_context_roots: []
EOF
  fi
  mkdir -p "${HM_DEV_BASE}/build-contexts"
}
