"""Vendored UniFi DPI application/category name catalog.

Sources:
  - Primary (apps):   ubntwiki cat_app.json
                      https://ubntwiki.com/products/software/unifi-controller/api/cat_app_json
  - Secondary (apps): unpoller dpi.go DPIApps map (fills gaps ubntwiki redacts)
                      https://raw.githubusercontent.com/unpoller/unifi/master/dpi.go
  - Categories:       ubntwiki cat_app.json category table (cross-checked against
                      unpoller dpi.go DPICats; both list the same 22 category ids)

Generated: 2026-06-21 (vendored snapshot; refresh ~yearly or when new IDs are observed)

Key scheme:
  APP_NAMES is keyed by compound int: (category << 16) + application
  CAT_NAMES is keyed by category int alone

Unknown IDs (any compound app key or category id absent from these tables) are
NOT invented here; resolve_app/resolve_cat fall back to the stringified raw ID.

This module contains ONLY data literals -- no logic. Import it from dpi_catalog.py.
Do not edit manually; regenerate from sources instead.
"""
