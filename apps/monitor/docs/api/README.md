...
**Development mode:** Auth is controlled by the `HOMELAB_MONITOR_DEV_AUTH` environment
variable (default: unset, auth disabled).

If `HOMELAB_MONITOR_DEV_AUTH=1`:
- All requests to protected endpoints must include the header `X-Auth: dev` (case-insensitive).
...
**STAGE-001-011 will replace this placeholder with real authentication.** The HTTP
middleware is designed to swap in an `AuthResolver` dependency for actual JWT/OAuth handling.
...
| 401 | `unauthorized` | Missing `X-Auth` header (auth enabled) or no `HOMELAB_MONITOR_DEV_AUTH`. |
...
**Auth:** Requires `X-Auth: dev` if `HOMELAB_MONITOR_DEV_AUTH=1`.

**Example curl:**
```bash
curl -s -H "X-Auth: dev" http://localhost:9090/api/collectors | jq .
```
...
**Example curl:**
```bash
curl -s -X POST -H "X-Auth: dev" http://localhost:9090/api/collectors/noop/retry | jq .
```
...
**Example curl (listen for 10 seconds):**
```bash
curl -s -H "X-Auth: dev" http://localhost:9090/api/events \
  --max-time 10 | head -20
```
...
| `HOMELAB_MONITOR_DEV_AUTH` | unset | Auth mode: unset (disabled) or `"1"` (require `X-Auth: dev` header). |
