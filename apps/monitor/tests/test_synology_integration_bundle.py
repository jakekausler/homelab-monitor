...
def test_register_all_registers_events() -> None:
    """register_all registers SynologyEventsCollector with the derived config."""
    loader = PluginLoader()
    register_all(loader)
    loaded = loader.load_all()
    records = [r for r in loaded if isinstance(r.collector, SynologyEventsCollector)]
    assert len(records) == 1
    record = records[0]
    assert isinstance(record, LoadedCollector)
