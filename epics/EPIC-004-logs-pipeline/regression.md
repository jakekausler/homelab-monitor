...
## STAGE-004-027 — Drain signature metrics + manual refresh API

- After each DrainConsumer cycle (inline in run_once, after snapshot), 6 metrics + a cardinality-warn gauge emit to VictoriaMetrics: homelab_drain_cycle_lines_total / _new_templates_total (counters), homelab_drain_cycle_duration_seconds (summary), homelab_log_signature_count{service_key,template_hash,severity} / _total / _first_seen_ts (gauges, from the per-cycle SignatureEvent stream), homelab_log_signature_cardinality_warn (0/1 at the configurable 100k threshold). severity comes from the per-line LogLine.severity ('unknown' if None). Gauges emit only for templates TOUCHED this cycle (no all-models iterator).
- Manual refresh API: POST /api/logs/signatures/refresh → 202 {cycle_id} (runs a cycle in a fire-and-forget background task, strong-ref'd to avoid GC), 409 if a cycle is already running (re-entrancy guard: asyncio.Lock in run_once + is_cycle_running; CycleInProgressError; run_forever skips the tick on conflict), 503 if drain disabled. GET /api/logs/signatures/refresh/{cycle_id} → 200 {status: running|done|failed, result?, error?}, 404 unknown/expired. Cycle results in an in-process CycleStatusStore (1h TTL, prune-on-access).
- Real-rig validation: apps/monitor/tests/integration/test_drain_metrics_integration.py asserts a real-VL cycle emits the metrics with correct labels (incl. severity) + the re-entrancy guard; auto-skips when the rig is absent. The HTTP-endpoint scenario skips when the rig backend's drain is disabled (covered by unit tests).
- Re-run check: make verify (drain_consumer.py / cycle_status.py / config.py / schemas.py / dependencies.py / routers/logs.py / lifespan.py / tail_metrics.py all 100%; openapi.json must include the two /api/logs/signatures/refresh endpoints + RefreshCycleResponse/RefreshStatusResponse/DrainCycleResultResponse models).

## STAGE-004-028 — Signature catalog backend + list/drill-in UI

- **Signatures tab load and routing:** Navigate to `/logs` → Signatures tab appears alongside Query tab. `/logs` redirects to `/logs/query` preserving deep-link search params (query history + saved queries continue to work via existing ln* params). Signatures tab loads from `/logs/signatures`.
- **Signatures list endpoint:** `GET /api/logs/signatures?service=<>&status=<>&label_q=<>&limit=100&offset=0` returns paginated list; desktop renders table with columns Service / Template / Count / Last Seen / Label / Status; mobile renders card-based layout (NOT table). Sticky table header is opaque (bg-white) on desktop; no header on mobile cards.
- **Signature detail route (deep-linked):** `/logs/signatures/{template_hash}/{service_key}` is a full-page route (NOT a side panel). Displays full template (monospace), 10 sample lines from VL, label/status controls, and "Open in Explorer" button. Back button (both desktop + mobile) returns to list. Selection persists across browser refresh (driven by URL, not transient state).
- **Label and status persistence:** `PATCH /api/logs/signatures/{template_hash}/{service_key}` with `{label?: string, status?: 'active'|'suppressed'|'expected'}` persists. Status-only PATCH does NOT clear an existing label (model_fields_set contract respected).
- **Sample lines endpoint:** `GET /api/logs/signatures/{template_hash}/{service_key}/samples?limit=10` returns live VL lines. Returns `{samples: [], reason: 'template_too_generic'}` for all-wildcard templates (no literal segments). Returns `{samples: [], reason: 'vl_unavailable'}` when VL is unreachable (never 500).
- **Open-in-Explorer deep-link:** Uses the template's non-wildcard segments with NO service/source_type scoping. Filter is `_msg:"seg1" AND _msg:"seg2" ...` (segments joined by AND, matching docker/systemd/cron/unknown services alike). Verify a systemd `*.service` signature's Open-in-Explorer returns matching logs (previously hardcoded `source_type:"docker"` excluded 81% of signatures).
- **Route-order regression:** `/api/logs/signatures/refresh` (from STAGE-004-027) is NOT swallowed by the `{template_hash}` catalog route. Verify POST /api/logs/signatures/refresh returns 202, not 404.
...
- **POST /api/logs/signatures/{template_hash}/{service_key}/annotations:** Submit `{note: string}` (1–2000 chars, whitespace-stripped, non-whitespace-only required). Author derived from session username (NOT client-supplied). Returns 201 with the new annotation object. Rejects 422 on empty/whitespace-only/>2000-char input. Returns 404 when the parent signature is absent (non-existent template_hash/service_key).
- **GET /api/logs/signatures/{template_hash}/{service_key}/annotations:** Returns list of annotations ordered newest-first (created_at DESC, id DESC); returns 200 `[]` (not 404) for an unknown signature; session required (401 anon), no CSRF token.
- **DELETE /api/logs/signatures/annotations/{id}:** Deletes the annotation by id. Returns 204 on success. Returns 404 when the id is absent OR when the id belongs to a different signature (composite-key scoping enforced; the row survives a wrong-signature delete attempt). CSRF required (403 without X-CSRF-Token header); session required (401 anon).
- **POST/DELETE require CSRF:** All mutation endpoints enforce X-CSRF-Token header; missing or invalid token returns 403. GET is session-only (no CSRF).
- **ON DELETE CASCADE verified:** Deleting a signature (via signatur list delete or admin CLI) cascade-deletes all its annotations (composite FK with PRAGMA foreign_keys=ON confirmed on the real DB connection path).
- **Migration 0035_log_signature_annotations:** Present in migration chain (down_revision 0034). Schema: `log_signature_annotations(id INTEGER PRIMARY KEY AUTOINCREMENT, template_hash TEXT NOT NULL, service_key TEXT NOT NULL, note TEXT NOT NULL, author TEXT NOT NULL, created_at TEXT NOT NULL)` with composite FK `(template_hash, service_key) REFERENCES log_signatures(template_hash, service_key) ON DELETE CASCADE` and indexes on `(template_hash, service_key)` and `created_at`. `EXPECTED_TABLES` in conftest includes log_signature_annotations (33 tables total).
- **Frontend add-button state:** Add button disabled while the note textarea is empty, whitespace-only, OR the POST mutation is pending. Textarea clears on successful add. Delete button shows per-item with confirm dialog (precedent: SavedQueriesPanel.tsx:68). Window.confirm() UX used for delete confirmation.
- **Re-run check:** `make verify` passes all backend tests (test_signature_annotations_repo.py, test_api_signature_annotations.py, test_db_migrations.py checking migration 0035 + EXPECTED_TABLES); frontend tests pass (SignatureDetailPage.test.tsx covering add/list/delete, button disabled states, clear-on-add, confirm dialog); 100% backend kernel coverage required (signature_annotations_repo.py, routers/logs.py annotation handlers).

## STAGE-004-031 — LogWindowFetcher shared service module
...
- vmalert rule ContainerCrashed (deploy/vmalert/metrics/container_crash.yaml) loads and fires on `increase(homelab_container_crash_total[5m]) > 0` with labels severity/source_tool:vmalert-metrics/category:container.
...
- vmalert rule ContainerUnhealthy (deploy/vmalert/metrics/container_healthcheck.yaml) loads + fires on increase(homelab_container_healthcheck_unhealthy_total[5m]) > 0 with labels severity/source_tool:vmalert-metrics/category:container.
...
- The `LogErrorRateCollector` must emit `homelab_container_error_rate{name=<service>}` GAUGE per service (one LogsQL `(severity:error OR critical OR fatal OR _msg:~"<patterns>") | stats by (service) count()` query; stateless; restart-safe). Re-verify via `tests/test_log_error_rate_collector.py` (12 tests, 100% collector coverage) + the config tests (`load_logs_config`).
- The vmalert rule file MUST be named `deploy/vmalert/metrics/error_rate_spike.yaml` (`.yaml`, NOT `.yml`) — vmalert-metrics globs `-rule=/etc/vmalert/metrics/*.yaml` and SILENTLY skips `.yml` files. A `.yml` rule file would never load (no parse error, just absent). [General invariant: ALL vmalert-metrics rule files must end `.yaml`.]
- The rule expr MUST use `clamp_min(5 * avg_over_time(homelab_container_error_rate[7d:5m]), 10)` for the per-service threshold — NOT `max(...)`. `max(vector, scalar)` is an AGGREGATION that collapses labels to `{}` and the comparison matches nothing (rule never fires). `clamp_min` is element-wise and preserves `{name}`. Re-verify by querying the rule's loaded expr (`/api/v1/rules`) contains `clamp_min` and that it returns per-`{name}` series.
- Spike-detection invariant: fires when `homelab_container_error_rate{name} > clamp_min(5×7d-baseline, 10)` — the 10/min static floor doubles as gap-robustness (a quiet/gappy service can't false-fire below 10 absolute). D-STATIC-FLOOR-AND-BASELINE.

## STAGE-004-038 — Anomaly Type D: Signature went silent (expected-silence allowlist)

- **Silence allowlist CRUD:** Create an `always` entry matching a silent signature → collector stops emitting `homelab_log_signature_silent` for it (verified via VictoriaMetrics query); `SignatureWentSilent` alert self-resolves.
- **Silence allowlist API validation:** `POST /api/logs/signatures/silence-allowlist` with `schedule_kind=cron` + valid cron expression → 201 (cron canonicalized via croniter); `schedule_kind=window` + start > end → 422 (invalid range); `schedule_kind=always` + non-empty schedule_value → 422 (unused value). `DELETE /{id}` removes entry; endpoint returns 204. `GET` returns all allowlist entries.
- **vmalert rule loads:** `deploy/vmalert/metrics/signature_silent.yaml` (note: `.yaml` not `.yml` — vmalert-metrics globs `*.yaml` only) loads in vmalert-metrics container with health=ok; rule fires on `homelab_log_signature_silent == 1`.
- **Frontend vertical scroll:** The Logs tab's SilenceAllowlistTab content wraps in a scrollable flex container (`min-h-0 flex-1 overflow-auto`) — scrolling the tab area does NOT clip content (regression guard for the overflow-hidden clipping bug).
...
- "Create alert from saved query" bakes the saved query's `selected_services` into the alert expr via `composeServicesExpr` (replicates backend `_compose_services_expr`: `service:"x" AND source_type:"y"` clauses, OR'd then AND'd with the query; empty base → just the services clause). Service-scoped saved queries (single OR multi-service) open in Advanced mode with the full service filter in the expr. Saved queries with NO selected_services keep prior simple/advanced behavior.
- "Create alert from signature" opens the modal in Advanced mode with `scaffoldLogsqlExpr(templateToLogsQl(template_str))`; deterministic rule_name (`deriveRuleName`) → re-launching the same shortcut surfaces the modal's existing duplicate-name warning (dedupe via deterministic naming; source_kind/source_ref NOT persisted).
- CreateAlertModal: the submit-path name error (client-side dup backstop + 409 conflict) is AUTHORITATIVE — it cancels the pending 300ms uniqueness debounce before setting, so a stale debounce timer cannot clobber the 409 "name already exists" warning under load. Regression guard: the 409 conflict warning must persist after submit.
- DESCOPED (future stage if wanted): Simple-mode multi-service selector in CreateAlertModal; spike-vs-baseline metricsql signature rule (036 render_signature_spike_rule needs a backend endpoint to be frontend-reachable).
- Test-harness: CreateAlertModal post-mutation assertions (409/400 inline errors) use `waitFor` (bounded retry) after an act-wrapped submit — required because the multi-hop async chain (zod resolver → mutateAsync reject → catch → setState) commits late under make-verify's concurrent backend+UI load. Do NOT revert to a bare synchronous getByTestId there.
