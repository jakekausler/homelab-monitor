...
- **Scope boundary with EPIC-022 (added 2026-06-16):** this epic owns ONLY cert *expiry* (`/etc/letsencrypt/live/*` walk, STAGE-014-001) and *served-cert TLS reachability* (STAGE-014-002). The **certbot RENEWAL-health** layer (last-successful-renewal age, renewal-cron failures), **per-subdomain public reachability probes** (every site in `nginx-configuator/sites-config.yaml`), **AWS Route 53 health**, **domain-registration expiry**, and **AWS spend** all live in the new **EPIC-022 (web/TLS/AWS)** — do NOT duplicate them here.
