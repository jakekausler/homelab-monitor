...
## Status: Complete (42 / 42 Complete)

## Stages Counter: 42 / 42 Complete

## Current Stage: — (EPIC-005 complete)

## Current Phase: — (EPIC-005 complete)
...
| STAGE-005-042 | HA cadence detail API + Status-tab idle/disabled cadence widget | ✓ Complete — Split off from 038 (D-CADENCE-SURFACE-SPLIT): the in-app cadence surface — a cadence detail endpoint + Status-tab widget/drill listing idle (>24h) + disabled automations + idle scripts, mirroring the `/entities` endpoint + `HaEntitiesDrill` pattern. Backend 3a+3b + frontend Desktop+Mobile. Runs AFTER 041 (the in-app widget is NOT a Grafana panel, so it is outside 041's Grafana-dashboard review scope — no forward-dependency violation). Depends 038 + 034; soft-depends 037. |
