...
| STAGE-018-013 | nginx-configuator integration: read `sites-config.yaml` (read-only) to enrich the targets table with public-domain mappings; "this internal target maps to public domain X" surfaced in inventory drill-downs. **NOTE (2026-06-16): this stage is INVENTORY-ENRICHMENT ONLY (map internal target → public domain). The ACTIVE per-subdomain reachability probing ("is each public site actually serving over HTTPS?") + certbot renewal-health + Route 53 health + domain expiry + AWS spend live in EPIC-022 (web/TLS/AWS). STAGE-018-012's ip-update Route-53-update timestamp is complemented by EPIC-022's Route-53-SIDE record/health confirmation. Don't build probing here.** |
