...
## STAGE-009-015 — fixer-runner egress enforcement

- [ ] `docker exec homelab-fixer-egress-proxy squid -k parse` — Squid config parses cleanly; exit 0. Regression if Squid version bump breaks config compatibility.
- [ ] `docker exec homelab-fixer-runner curl -s -o /dev/null -w '%{http_code}' https://api.anthropic.com/v1/messages` — returns a real HTTP status (4xx from Anthropic auth challenge), NOT 000 (timeout) or 403 (Squid deny). Regression if baseline allow-list breaks.
- [ ] `docker exec homelab-fixer-runner curl -s -o /dev/null -w '%{http_code}' https://example.com/` — returns 000 with Squid access.log line `TCP_DENIED/403 CONNECT example.com:443 HIER_NONE/-`. Regression if off-list endpoints leak through.
- [ ] `docker exec homelab-fixer-runner timeout 5 curl --noproxy '*' https://1.1.1.1/` — times out (curl exit 28 or 124). Confirms nftables `homelab_fixer` table drops raw TCP bypass of the proxy. Regression if the nftables rule is dropped by a host firewall reset.
- [ ] `docker exec homelab-fixer-egress-proxy stat -c '%i' /etc/squid/allowlist.txt` vs `stat -c '%i' /storage/programs/homelab-monitor/deploy/compose/proxy/allowlist.txt` — inodes match. Regression if a maintenance operation (e.g. `sed -i`) swaps the inode: Squid inside the container would see stale content while host sees new.
- [ ] `docker network inspect homelab-monitor-fixer-egress --format '{{index .Options "com.docker.network.bridge.name"}}'` returns `hm-fixegress`. Regression if compose was edited to lose the `driver_opts.com.docker.network.bridge.name` field — the nftables rule targets that specific interface name.
- [ ] `docker inspect homelab-fixer-egress-proxy --format '{{range $k, $v := .NetworkSettings.Networks}}{{$k}}={{$v.GwPriority}} {{end}}'` shows `fixer-uplink=100` and `fixer-egress=0`. Regression if `gw_priority` fields are dropped from compose — the proxy's default route would go via fixer-egress and be dropped by nftables, breaking outbound entirely.
