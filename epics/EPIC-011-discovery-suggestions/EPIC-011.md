...
**None.** Discovery events are `suggestions`, not `alerts`, and they don't flow through the alert-outcome path that EPIC-010 depends on. No integration work required. If a future stage adds an alert-emitting discovery signal (e.g., "unexpected new container detected — alert on this"), that stage should follow the standard EPIC-010 rules for source_tool labeling like any other alert source.
...
- Kept as-is and reused by the global inbox when the kind is docker\_\*
