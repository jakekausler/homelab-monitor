...
- [ ] `deploy/vmalert/metrics/ssh.yaml` (group `ssh_health`) loads into vmalert-metrics with no parse error; all 3 rules (`SshTargetUnreachable`, `SshHostKeyMismatch`, `SshProbeStale`) appear in `/api/v1/rules` with `health=ok`.
- [ ] Severities: `SshTargetUnreachable`=warning, `SshHostKeyMismatch`=critical, `SshProbeStale`=warning. All carry `source_tool: vmalert-metrics` + `target_kind: ssh_target`. `routing_channel: monitoring-health` on `SshTargetUnreachable` + `SshProbeStale` only (NOT on `SshHostKeyMismatch` — it's a security finding, default-routed).
- [ ] Exprs are plain per-target comparisons (NO `absent()` — multi-target absent() collapses labels): `homelab_ssh_up == 0` (for 5m), `homelab_ssh_host_key_mismatch == 1` (for 0s), `homelab_ssh_last_success_age_seconds > 900` (for 5m).
